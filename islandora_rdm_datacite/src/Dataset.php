...
class Dataset {
  ...
  public function createFromNode(NodeInterface $node) {
    ...
    if ($identifier_field = $node->get('field_islandora_rdm_identifier')
      ->first()) {
    ...
    if ($resource_type_field = $node->get('field_rdm_resource_type')->first()) {
      ...
      if ($resource_type_general_field = $node->get('field_rdm_resource_type_general')
        ->first()) {
    ...
    if ($additional_date_fields = $node->get('field_date_paragraph')) {
      foreach ($additional_date_fields as $additional_date_field) {
        if ($date_target = $additional_date_field->get('entity')->getTarget()) {
          $additional_date_string = $date_target->get('field_date_string')
            ->getString();
          $additional_date_type = $date_target->get('field_type_of_date')
            ->getString();
    ...
    if ($descriptions_field = $node->get('field_rdm_description')) {
      foreach ($descriptions_field as $description_field) {
        if ($description_target = $description_field->get('entity')
          ->getTarget()) {
          $description_string = $description_target->get('field_rdm_description')
            ->getString();
          $description_type = $description_target->get('field_rdm_description_type')
            ->getString();
    ...
    if ($location_data = $node->get('field_physical_location')) {
      if ($location_data->count()) {
        ...
        foreach ($location_data as $location_datum) {
          ...
          if ($location_target = $location_datum->get('entity')->getTarget()) {
            $location_string = $location_target->get('field_geolocation')
              ->getString();
            if ($location_string) {
              $geolocationplace = $this->doc->createElement('geoLocationPlace', $location_string);
              $geolocation_element->appendChild($geolocationplace);
            }
            $coordinates = $location_target->get('field_geolocationpoint_')
              ->getValue();
            if ($coordinates) {
              $geolocationpoint = $this->doc->createElement('geoLocationPoint');
              $geolocation_element->appendChild($geolocationpoint);
              $long = $this->doc->createElement('pointLongitude', $coordinates[0]['lng']);
        ...
        $resource->appendChild($geolocations_element);
      }
    }
    ...
    foreach ($node->get($field_name) as $reference_field) {
      if ($researcher_target = $reference_field->get('entity')->getTarget()) {
        ...
        if ($is_contributor = $researcher->getType() == 'rdm_contribution_personal') {
          $contributor_type = $researcher->get('field_rdm_role_personal')
            ->first()
            ->getString();
          $wrapper_element->setAttribute('contributorType', $contributor_type);
          $researcher = $researcher->get('field_rdm_person')
            ->first()
            ->get('entity')
            ->getTarget()
            ->getValue();
        ...
        elseif ($is_contributor = $researcher->getType() == 'rdm_contribution_organizational') {
          $contributor_type = $researcher->get('field_rdm_role_organizational')
            ->first()
            ->getString();
          $wrapper_element->setAttribute('contributorType', $contributor_type);
          $researcher = $researcher->get('field_rdm_organization')
            ->first()
            ->get('entity')
            ->getTarget()
            ->getValue();
        ...
        // Person.
        if ($is_person) {
          $name = $researcher->get('field_rdm_personal_name')
            ->first()
            ->getString();
        ...
        // Organization.
          ...
          $name = $researcher->get('field_rdm_organization_name')
            ->first()
            ->getString();
          $name_element = $this->doc->createElement($element_name . 'Name', $name);
      ...
      if ($is_person) {
        foreach ($researcher->get('field_rdm_personal_identifier') as $person_id) {
          ...
          if (!empty($person_id->get('field_rdm_identifier_string')->first()) &&
            $id = $person_id->get('field_rdm_identifier_string')
              ->first()
              ->getString()) {
            $scheme = $person_id->get('field_rdm_identifier_type_person')
              ->first()
              ->getString();
      ...
      // Identifiers (org)
        ...
        foreach ($researcher->get('field_rdm_organizational_ids') as $org_id) {
          ...
          if (!empty($org_id->get('field_rdm_identifier_string')->first()) &&
            $id = $org_id->get('field_rdm_identifier_string')
              ->first()
              ->getString()) {
            $scheme = $org_id->get('field_rdm_identifier_type_org')
              ->first()
              ->getString();
            $id_element = $this->doc->createElement('nameIdentifier', $id);
            $id_element->setAttribute('nameIdentifierScheme', $scheme);
      ...
      // Affiliation (person)
      if ($is_person && !empty($researcher->get('field_rdm_personal_affiliation')
          ->first())) {
        $affiliation = $researcher->get('field_rdm_personal_affiliation')
          ->first()
          ->get('entity')
          ->getTarget();
        $affiliation_name = $affiliation->get('field_rdm_organization_name')
          ->first()
          ->getString();
